        ...
        for sep in numpy.array(sent_tokenize(document.lower())):
            ...
            for token in sentence:
                if token in embeddingVectors[publisher]:
                    embeddingVectors[publisher][token.text].append(
                        token.embedding.tolist()
                    )
                else:
                    embeddingVectors[publisher][token.text] = [token.embedding.tolist()]

        # save embedding
        Path(os.path.join(output, "embedding", publisher)).mkdir(
            parents=True, exist_ok=True
        )

        with open(
            os.path.join(output, "embedding", publisher, "embeddingVector.json"), "w+"
        ) as fp:
...
def saveEmbedding(embeddingVectors, output):
    ...
    with open(
        os.path.join(output, "embedding", f"{KEYWORD}_EmbeddingVectors.json"), "w+"
    ) as fp:
...
def loadEmbedding(publishers, output):
    ...
    for publisher in publishers:
        embedding_path = os.path.join(
            output, "embedding", publisher, "embeddingVector.json"
        )
    ...
    with open(
        os.path.join(output, "embedding", f"{token}_EmbeddingVectors.json"), "w+"
    ) as fp:
